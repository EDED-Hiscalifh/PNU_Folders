...
# ===================== Problem 5 =====================
# The things need to consider : Repeat Q1-Q4 for each set, where 5 classification methods such as LR, LDA, QDA, N
B, and KNN.
# Required outputs : Find the average ACC, average F1 score and average MCC of the test samples over 100 differen
t sets.
...
for (i in 1:100){
  ...
  colnames(model.err) <- c('ACC', 'F1', 'MCC'); rownames(model.err) <- K
  for (m in 1:length(K)) {
    # First, find the optimal K values that maximize ACC, F1 score and MCC of the validation samples, respectivel
    y.
    # Make prediction of validation set
